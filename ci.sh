...
# install ccache for compile.
sudo apt-get install -y --no-install-recommends ccache

# set cache for compile.
ccache -M 15G
...
function do_binutils() {
    "$base"/build-binutils.py \
        ...
        --show-build-commands \
        --targets arm-linux-gnueabi aarch64-linux-gnu
}
...
builder.matrix = {'defconfig': ['arm64']}
...
function do_llvm() {
    ...
    "$base"/build-llvm.py \
        --vendor-string "AOSP-Kernel-Build-Test" \
        --assertions \
        --build-stage1-only \
